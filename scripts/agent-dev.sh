...
# "nas-port-mcp-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "nas-port-mcp-dev" is overwritten by
...
PROJECT_NAME="nas-port-mcp"
AGENT_NAME="nas-port-mcp-dev"
WORKSPACE="/workspaces/nas-port-mcp"
...
if [ -x "/workspaces/nas-port-mcp/scripts/agent-dev.sh" ]; then
  "/workspaces/nas-port-mcp/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
