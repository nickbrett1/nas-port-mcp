...
agy-dev() {
  ...
  echo "Starting Antigravity with Doppler (common + nas-port-mcp)..."
  ...
  doppler run --project common --config dev -- doppler run --forward-signals --project nas-port-mcp --config dev -- agy "$@"
}
...
if [[ "$PWD" == "$HOME" ]]; then
  cd /workspaces/nas-port-mcp 2>/dev/null
fi
